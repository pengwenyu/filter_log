...
nums = [210,210,220,220,210,220,210]#[0,0,0,0,0,0,0]###############Add different noise
nums0 = nums[0]
for k in range(nums0):###### repeated events
...
for k in range(nums2):  ##########Exclude tail
    trace = traces[k]
    start = idx[trace]
    end = idx[trace+1]
    ignore = 0
...
for k in range(nums3):  ##########Missing episode
    trace = traces[k]
    start = idx[trace]
    end = idx[trace+1]
    ignore = 0
...
for k in range(nums4): ###############Pertubed order
    trace = traces[k]
    start = idx[trace]
    end = idx[trace+1]
    locate = 0
...
for k in range(nums6): ##### Double event
    trace = traces[k]
    #print("TA",trace)
    start = idx[trace]
    end = idx[trace+1]
